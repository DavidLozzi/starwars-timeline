...
CI (`.github/workflows/node.js.yml`) runs, in order: `npm ci` → `npm run lint` → `npm test` → `node prepJson.js` (from `build_scripts/`) → `npm run build` → `node prerenderCharacters.js` (from `build_scripts/`) → copies `build/*` into `docs/` and commits. `docs/` is the actual GitHub Pages output directory — it's checked in and regenerated by CI, not built locally as part of normal dev.
...
- The script has **zero dependencies** — CI only runs `npm ci` at the repo root and never installs `build_scripts/node_modules`.
- `index.html` must keep its `<!-- PRERENDER:BODY:START/END -->` markers and its `<link rel="canonical" href="https://timeline.starwars.guide/" />` line; both `prepJson.js` and the prerenderer anchor on them and throw if they're gone.
- Injected `<meta>` tags carry `data-react-helmet="true"` so react-helmet **replaces** rather than duplicates them on mount; the canonical `<link>` deliberately does not (helmet emits no link tags here). Head tags in `src/pages/Home/index.jsx` and in the prerenderer must stay in sync — both read the `metaDescription` that `prepJson.js` precomputes into `characters.json`.
...
2. `build_scripts/prepJson.js` reads `data.json` plus `build_scripts/character_descriptions.json` (AI-generated bios/timelines, produced separately by `build_scripts/description.js` using Claude) and computes derived, denormalized output:
...
3. **To add/edit timeline content (a character, movie, era, etc.), edit `build_scripts/data.json`, then run `node prepJson.js` from `build_scripts/` to regenerate everything in `src/data/`.** Do not hand-edit `src/data/*.json` directly — it will be overwritten. While `npm start` is running, a dev-only Vite plugin in `vite.config.js` (`prepJsonOnEdit`) reruns `prepJson.js` automatically on every save of `data.json` or `character_descriptions.json`; a failed run (usually a half-edited, invalid `data.json`) is logged in the dev-server output and retried on the next save.
4. **Adding a new character**: after adding its entry to `data.json` (with a `wookiepedia` URL), also run `node description.js` from `build_scripts/` to generate its bio/timeline into `character_descriptions.json` before running `prepJson.js`. It needs `ANTHROPIC_API_KEY` in `build_scripts/.env` (see `.env.sample`).
   - `description.js` runs **Claude Opus 5.5** (`claude-opus-5-5`; override per run with `DESCRIPTION_MODEL=…`). Wookieepedia answers the model's own fetches with HTTP 402/403, so the script fetches each article itself through `api.php` (raw revision via `action=query&prop=revisions` — `action=parse` times out on huge articles like Anakin — one request a second, honest user agent — `/wiki/` HTML is blocked), strips `<ref>`s, caps it at 40,000 characters and puts it in the prompt; web search stays available for gaps. It sends the character's **whole `data.json` entry** plus a field guide explaining our conventions (negative years are BBY, `startYear` vs `birthYear`, what `startYearUnknown` / `endYearUnknown` actually mean, and that A New Hope and Rogue One sit at **1 BBY (-1)** per Wookieepedia's current Galactic Atlas dating), and asks Claude to verify the dates — especially the ones we present as known — before writing anything. With the article in the prompt a character takes ~30–60s and ~$0.25.
   - Local `--via-claude-code` runs go through `@anthropic-ai/claude-agent-sdk`, which bundles its own Claude Code build. A new model needs a new enough SDK: Opus 5.5 needs 0.3.280+, and an older one fails every character with "result was not valid JSON", hiding the real `API Error: 400 … does not support this model`.
...
**Social cards**: `build_scripts/socialCards.js` (local only — needs `sharp`, which CI never installs) composites each character's default `imageUrl` into the magenta `#FF00FF` box of `build_scripts/social-template.png` and writes the name in the A New Hope face (`build_scripts/fonts/ANewHope.ttf`, converted from the woff2; no accented glyphs, so names are de-accented). Output is `public/social/social_<Title_with_underscores>.jpg`, committed. The name band's width comes from the box and its size is capped at what "Tobias Beckett" renders at, so short names never outgrow it; only `NAME_LEFT` / `NAME_CENTER_Y` are tied to the template's lettering. `prepJson.js` sets each character's `socialImage` when its card exists, and both `prerenderCharacters.js` and react-helmet in `src/pages/Home` use it, falling back to `/social.png`. Rerun `node socialCards.js ["Name" …]` after changing a portrait, adding a character, or editing the template.
...
- **Header wordmark**: the `<h1>` in `src/pages/Home/index.jsx` wears the hub's two-script wordmark treatment (`../starwars-guide/_includes/header.html`) with this app's own words — "Ultimate Star Wars Timeline" in the `A New Hope` face, repeated beside it in `AurebeshAF` grey. The Aurebesh twin sits on the line below (the h1 is a `flex-direction: column`) and is a `h1::after` reading `content: attr(data-aurebesh)`, **not** a second span: the h1 must keep exactly one text node, or the page's only heading hands Googlebot a visible duplicate of its own title (and `App.test.jsx`'s `getByText` would match two elements). The heading carries no `text-shadow` — the old pill-bezel glow reads as puffy under a display face. Styles live in both themes' `elements.header` (`src/themes/jedi.js`, `sith.js`); keep them in sync.
- **Brand webfonts**: `public/fonts/ANewHope.woff2` and `AurebeshAF-Canon.woff2`, copied from the hub's `assets/fonts/`. Both `@font-face` blocks sit in the root `index.html` `<style>` — deliberately **not** scoped to `html.booting` like the rest of that block, and not in styled-components, since a `<link rel=preload>` whose face is only declared once the bundle parses is dropped and warned about. Both are `font-display: block` and preloaded, the same three-part fix the hub made: without it the wordmark flashes a fallback, and the Aurebesh face has no sane fallback at all (it renders plain latin letters). Filenames aren't content-hashed, so replace a face by renaming the file and updating the preload and the `@font-face` together.
- **The "Created By" menu item is the hub credit, not a social link** (hub convention — see `../starwars-guide/CLAUDE.md`). `src/organisms/MainMenu/index.jsx`, the `note` item at the bottom of the main menu: the app is what gets shared and bookmarked, not its landing page, so it is the only path from the timeline back to the brand. It must point at `https://starwars.guide/?utm_source=timeline&utm_medium=app&utm_campaign=credit` — UTM-tagged because it leaves the app, which is the one exception to the hub's no-UTMs-on-internal-links rule. It pointed at `twitter.com/aurebeshfiles` until 2026-09-18: wrong destination, and a handle dead since X moved to `starwars_77_af` on 2026-08-18. Don't repoint it at a social account.
- **Hub links must be the exact canonical URL.** Every top-level hub page canonicalises to a trailing slash (`/games/`, `/support-aurebesh-files/`, `/star-wars-timeline/`); the bare and `.html` forms 301. Same rule the character generator follows, and it applies to hand-written links in the app UI too — three in `MainMenu` were hopping until 2026-09-18.
